...
@include('site.partials.seo', ['seoTitle' => 'Get a Brisbane Tiling Quote | Ozghan.au', 'seoDescription' => 'Request a quote from Ozghan for bathroom, kitchen, floor, outdoor or commercial tiling in Brisbane.'])
...
.calendar-popover{
  display:none; position:absolute; left:0; right:0; top:calc(100% + 8px); z-index:1000;
  background:var(--white); border:1px solid var(--line-strong); border-radius:var(--radius-sm);
  padding:16px; box-shadow:0 16px 32px rgba(35,32,28,0.14);
}
...
.calendar-toolbar strong{ font-family:var(--font-display); font-size:1rem; }
.calendar-nav{ width:32px; height:32px; padding:0; border:1px solid var(--line-strong); background:var(--bg); color:var(--ink); border-radius:var(--radius-sm); cursor:pointer; }
.calendar-nav:hover{ border-color:var(--clay); color:var(--clay-dark); }
...
.calendar-weekdays span{ text-align:center; font-family:var(--font-mono); font-size:0.66rem; color:var(--ink-soft); }
.calendar-day{ min-height:34px; padding:0; border:1px solid transparent; background:transparent; color:var(--ink); border-radius:var(--radius-sm); cursor:pointer; font:500 0.82rem var(--font-body); }
.calendar-day:hover{ border-color:var(--clay); background:var(--clay-tint); }
.calendar-day.is-today{ border-color:var(--clay); }
.calendar-day.is-selected{ background:var(--clay); color:var(--white); border-color:var(--clay); }
.calendar-day.is-muted{ color:#AAA39A; cursor:default; }
.calendar-day:disabled{ color:#C8C1B8; cursor:not-allowed; background:transparent; }
...
.quote-page{ display:flex; flex-direction:column; min-height:100vh; }
.quote-page .site-header{ z-index:1001; }
.quote-page .nav-links{ z-index:1002; }
.quote-page #main{ display:none; }
.quote-page .site-footer{ display:block; order:3; }
.quote-page #quote-modal{
  position:static; display:flex; align-items:flex-start; justify-content:center;
  order:2; flex:1; min-height:calc(100vh - 100px); padding:48px 20px; background:var(--bg);
}
.quote-page #quote-modal .modal{ max-height:none; }
.quote-page #quote-modal .modal-close{ display:none; }
.quote-page #quote-modal .modal{
  max-width:720px; border-top-width:8px; overflow:visible; box-shadow:0 18px 48px rgba(35,32,28,0.14);
}
.quote-page #quote-modal .modal-head{ padding:32px 36px 0; }
.quote-page #quote-modal .modal-head p{ max-width:52ch; margin:8px 0 0; font-size:0.9rem; }
.quote-page #quote-modal .modal-body{ padding:28px 36px 36px; }
.quote-page #quote-modal .modal-body{ overflow:visible; }
.quote-page #quote-modal .modal-steps{ padding-left:36px; padding-right:36px; }
.quote-page #quote-modal .modal-steps span{ height:4px; }
@media (max-width:560px){
  .quote-page #quote-modal{ padding:28px 12px; }
  .quote-page #quote-modal .modal-head,
  .quote-page #quote-modal .modal-body{ padding-left:22px; padding-right:22px; }
  .quote-page #quote-modal .modal-steps{ padding-left:22px; padding-right:22px; }
}
...
<header class="site-header">
  <div class="container nav">
    ...
    <nav class="nav-links" aria-label="Primary">
      ...
      <a href="/contact" data-nav="contact" aria-current="page">Contact</a>
    </nav>
    <div class="nav-cta">
...
<section>
  <div class="container contact-grid">
    <div>
      ...
      <h2>Get in touch</h2>
      <p>For a full quote with a date and address, use <button type="button" class="card-link" style="background:none;border:0;padding:0;font:inherit;color:var(--clay-dark);text-decoration:underline;cursor:pointer;" data-open-quote>Get a Quote</button> instead — this form is for general questions.</p>
...
function initQuoteModal() {
  ...
  if (photoInput) photoInput.addEventListener('change', async () => {
    ...
    const remaining = 5 - state.photoUploads.length;
    if (remaining <= 0) { photoInput.value = ''; return; }
    if (files.length > remaining) files = files.slice(0, remaining);
    const upload = new FormData();
    files.forEach(file => upload.append('photos[]', file));
    const list = photoInput.closest('.field').querySelector('.photo-list');
    list.textContent = 'Uploading photos…';
    try {
      const response = await fetch('/orders/photos', { method: 'POST', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' }, body: upload });
      if (!response.ok) throw new Error('Photo upload failed');
      const result = await response.json();
      state.photoUploads.push(...result.photos);
      state.photos = state.photoUploads.map(photo => photo.name);
      list.innerHTML = state.photoUploads.map(photo => '<span class="photo-preview"><img src="' + photo.url + '" alt="' + photo.name.replace(/"/g, '&quot;') + '" title="' + photo.name.replace(/"/g, '&quot;') + '"><button type="button" data-photo-path="' + photo.path + '" aria-label="Remove ' + photo.name.replace(/"/g, '&quot;') + '">×</button></span>').join('');
      list.querySelectorAll('[data-photo-path]').forEach(button => button.addEventListener('click', async () => { const remove = new FormData(); remove.append('path', button.dataset.photoPath); const response = await fetch('/orders/photos/remove', { method: 'POST', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' }, body: remove }); if (response.ok) { state.photoUploads = state.photoUploads.filter(photo => photo.path !== button.dataset.photoPath); state.photos = state.photoUploads.map(photo => photo.name); button.closest('.photo-preview').remove(); } }));
      photoInput.value = '';
  ...
  function initDatePicker() {
    const picker = overlay.querySelector('#date-picker');
    if (!picker) return;
    const input = picker.querySelector('#quote-date');
    const popover = picker.querySelector('#calendar-popover');
    const grid = picker.querySelector('#calendar-grid');
    const label = picker.querySelector('#calendar-label');
    const previous = picker.querySelector('#calendar-prev');
    ...
    const displayDate = (date) => date.toLocaleDateString('en-AU', {
      day: 'numeric', month: 'long', year: 'numeric'
    });

    function renderCalendar() {
      label.textContent = month.toLocaleDateString('en-AU', { month: 'long', year: 'numeric' });
      grid.innerHTML = '';
      const firstDay = new Date(month.getFullYear(), month.getMonth(), 1).getDay();
      const daysInMonth = new Date(month.getFullYear(), month.getMonth() + 1, 0).getDate();
      const monthStart = new Date(month.getFullYear(), month.getMonth(), 1);
      previous.disabled = monthStart <= new Date(today.getFullYear(), today.getMonth(), 1);
      ...
      for (let day = 1; day <= daysInMonth; day += 1) {
        ...
        if (dateValue === iso(today)) button.classList.add('is-today');
        if (dateValue === selected) button.classList.add('is-selected');
        if (date < today) {
          ...
          button.addEventListener('click', () => {
            ...
            input.dataset.dateValue = dateValue;
    ...
    function toggleCalendar(force) {
      const open = typeof force === 'boolean' ? force : !popover.classList.contains('open');
    ...
    document.addEventListener('click', (event) => {
      if (!picker.contains(event.target)) toggleCalendar(false);
    });
    renderCalendar();
  }
