...
<div class="admin-card order-chart-card">
    ...
    <div class="chart-wrap">
        <svg viewBox="0 0 {{ $orderChart['chartWidth'] }} {{ $orderChart['chartHeight'] }}" role="img" aria-label="Orders over the last 14 days">
            <defs><linearGradient id="orders-area" x1="0" x2="0" y1="0" y2="1"><stop offset="0" stop-color="#4091C5" stop-opacity=".35"/><stop offset="1" stop-color="#4091C5" stop-opacity=".03"/></linearGradient></defs>
            ...
            <polyline points="{{ $orderChart['linePoints'] }}" fill="none" stroke="#4091C5" stroke-width="4" stroke-linecap="round" stroke-linejoin="round" />
            @foreach($orderChart['orderChart'] as $index => $day)
                @if($day['count'] > 0)
                    @php($point = $orderChart['points'][$index])
                    <circle cx="{{ $point['x'] }}" cy="{{ $point['y'] }}" r="6" fill="#fff" stroke="#4091C5" stroke-width="3"><title>{{ $day['count'] }} order(s) on {{ $day['label'] }}</title></circle>
                    <text x="{{ $point['x'] }}" y="{{ $point['y'] - 12 }}" text-anchor="middle" fill="#4091C5" font-size="12" font-weight="bold">{{ $day['count'] }}</text>
                @endif
